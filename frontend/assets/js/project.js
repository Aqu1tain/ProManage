document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadProject(projectId) {
      try {
        const response = await authenticatedFetch(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.PROJECTS}/${projectId}`);
        ...
        if (response.ok && data.status === 'success') {
          ...
          projectDescription.textContent = project.description || 'Aucune description';
          ...
          console.error('Erreur lors du chargement du projet:', data.message);
    ...
    async function loadTasks(projectId) {
      try {
        ...
        todoTasks.innerHTML = '<div class="loading">Chargement des tâches...</div>';
        inProgressTasks.innerHTML = '<div class="loading">Chargement des tâches...</div>';
        completedTasks.innerHTML = '<div class="loading">Chargement des tâches...</div>';
        
        const response = await authenticatedFetch(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.PROJECTS}/${projectId}/tasks`);
        const data = await response.json();
        ...
        if (response.ok && data.status === 'success') {
          ...
          if (tasks.length === 0) {
            todoTasks.innerHTML = '<div class="empty-state">Aucune tâche</div>';
            inProgressTasks.innerHTML = '<div class="empty-state">Aucune tâche</div>';
            completedTasks.innerHTML = '<div class="empty-state">Aucune tâche</div>';
            return;
          }
          ...
          todoTasks.innerHTML = '<div class="error">Erreur lors du chargement des tâches</div>';
          inProgressTasks.innerHTML = '<div class="error">Erreur lors du chargement des tâches</div>';
          completedTasks.innerHTML = '<div class="error">Erreur lors du chargement des tâches</div>';
        }
      } catch (error) {
      ...
      div.innerHTML = `
        ...
        <div class="task-footer">
          ...
          <div class="task-actions">
            <button class="btn btn-secondary edit-task" data-id="${task.id}">Modifier</button>
            <button class="btn btn-danger delete-task" data-id="${task.id}">Supprimer</button>
          </div>
        </div>
      `;
    ...
    async function saveTask() {
      ...
      try {
        ...
        if (response.ok && data.status === 'success') {
          ...
          console.error('Erreur lors de l\'enregistrement de la tâche:', data);
          alert(`Erreur: ${data.message || 'Impossible d\'enregistrer la tâche'}`);
        }
      } catch (error) {
    ...
    async function deleteTask(taskId) {
      try {
        const response = await authenticatedFetch(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.TASKS}/${taskId}`, {
          method: 'DELETE'
        });
        ...
        if (response.ok && data.status === 'success') {
          ...
          alert(`Erreur: ${data.message || 'Impossible de supprimer la tâche'}`);
        }
      } catch (error) {
