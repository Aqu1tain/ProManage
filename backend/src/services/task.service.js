...
class TaskService {
    ...
    async createTask(taskData, userId) {
        ...
        if (user.role === roles.CONTRIBUTOR) {
            ...
            if (assigned_to && assigned_to !== userId) {
                throw new AppError(
                    "En tant que contributeur, vous ne pouvez créer des tâches que pour vous-même",
                    403
                );
            }
        }
        ...
        if (assigned_to) {
            ...
            if (assignedUser.team_id !== project.team_id) {
                throw new AppError(
                    "L'utilisateur assigné n'appartient pas à l'équipe du projet",
                    400
                );
            }
        }
    ...
    async updateTask(taskId, updateData, userId) {
        ...
        // Pour les contributeurs, on vérifie qu'ils sont autorisés à modifier la tâche
        if (user.role === roles.CONTRIBUTOR) {
            ...
            // On empêche uniquement la réassignation
            if (
                updateData.assigned_to &&
                updateData.assigned_to !== task.assigned_to
            ) {
                throw new AppError(
                    "En tant que contributeur, vous ne pouvez pas réassigner une tâche",
                    403
                );
            }
        }

        // Vérifier si assigned_to est mis à jour
        ...
        // Mettre à jour la tâche : cela mettra à jour tous les champs présents, y compris "status"
    ...
    async updateTaskStatus(taskId, status, userId) {
        ...
        if (user.role === roles.CONTRIBUTOR) {
            ...
            if (task.assigned_to !== userId) {
                throw new AppError(
                    "Vous n'avez pas l'autorisation de changer le statut de cette tâche",
                    403
                );
            }
        }
