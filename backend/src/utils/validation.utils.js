...
exports.authSchemas = {
    register: Joi.object({
        ...
        role: Joi.string()
            .valid("admin", "project_manager", "contributor")
            .default("contributor")
            .messages({
                "any.only":
                    "Le rôle doit être admin, project_manager ou contributor",
            }),
        team_name: Joi.string().when("role", {
            is: "project_manager",
            then: Joi.string().min(3).max(50).required().messages({
                "string.min":
                    "Le nom d'équipe doit contenir au moins 3 caractères",
                "string.max":
                    "Le nom d'équipe ne peut pas dépasser 50 caractères",
                "any.required":
                    "Le nom d'équipe est requis pour un chef de projet",
            }),
        }),
    }),
    ...
    join: Joi.object({
        ...
        invitation_key: Joi.string().required().messages({
...
exports.taskSchemas = {
    create: Joi.object({
        title: Joi.string().min(3).max(100).required().messages({
            "string.min":
                "Le titre de la tâche doit contenir au moins 3 caractères",
            "string.max":
                "Le titre de la tâche ne peut pas dépasser 100 caractères",
        ...
        assigned_to: Joi.string().uuid().messages({
            "string.guid":
                "L'ID de l'utilisateur assigné doit être un UUID valide",
        }),
    ...
    update: Joi.object({
        ...
        // Ajout du champ status dans le schéma de mise à jour
        status: Joi.string()
