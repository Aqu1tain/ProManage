...
exports.isProjectMember = async (req, res, next) => {
    try {
        let projectId = req.params.id || req.body.project_id;

        // Détecter si nous sommes dans un contexte de tâche
        const isTaskRoute = req.originalUrl.includes("/tasks/");

        if (isTaskRoute && req.params.id) {
            // Si c'est une route de tâche avec un ID, chercher d'abord la tâche
            const Task = require("../models/sql/task.model");
            const task = await Task.findByPk(req.params.id);

            if (!task) {
                return next(new AppError("Tâche non trouvée", 404));
            }

            // Utiliser le projet associé à la tâche
        ...
        if (
            ...
            project.team_id === req.user.team_id
        ) {
            return next();
        }
